...
echo "=================================================="
echo "UPGRADE DASHBOARD + REALTIME WITH USEFUL TECHNICAL CHARTS"
echo "=================================================="

mkdir -p .backup-technical-charts
for f in app/page.tsx app/monitoring/page.tsx app/analytics/page.tsx; do
  [ -f "$f" ] && cp "$f" ".backup-technical-charts/$(echo "$f" | tr '/' '_').bak"
done

mkdir -p components

cat > components/technical-chart-grid.tsx <<'TSX'
"use client";
...
targets = [
    ...
    ("app/monitoring/page.tsx", "realtime"),
...
for file, scope in targets:
    ...
    import_line = 'import { TechnicalChartGrid } from "@/components/technical-chart-grid";'
    ...
    marker = f'<TechnicalChartGrid scope="{scope}" />'

    if marker not in s:
        if "</main>" in s:
            s = s.replace("</main>", f"      {marker}\n    </main>", 1)
        else:
            print(f"WARNING: {file} has no </main>, not injected")
...
echo "=================================================="
echo "DASHBOARD / REALTIME / ANALYTICS CHARTS PATCHED"
echo "=================================================="
